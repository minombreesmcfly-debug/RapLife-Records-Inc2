...
import { 
  ...
  ArrowDownRight, Instagram, X, Square, Circle, Download, RefreshCw, RotateCcw
} from 'lucide-react';
import { collection, query, orderBy, limit, getDocs, getDoc, addDoc, doc, where, serverTimestamp } from 'firebase/firestore';
...
export default function SpotifyTurntable() {
  ...
  // --- 1. FETCH BEATS & INSTRUMENTALS (LOCAL ASSETS & CLOUD) ---
  const fetchInstrumentals = async () => {
    setLoadingTracks(true);
    try {
      let combinedBeats: Track[] = [];

      // A. Fetch local beats from /api/beats-local (auto-scans /assets for any beat)
      try {
        const res = await fetch('/api/beats-local');
        if (res.ok) {
          const ct = res.headers.get('content-type') || '';
          if (ct.includes('application/json')) {
            const localBeats = await res.json();
            if (Array.isArray(localBeats)) {
              combinedBeats = localBeats.map((b: any) => ({
                id: b.id,
                artistId: b.artistId || 'raplife-beatmaker',
                artistName: b.artistName || 'RAPLIFE PROD',
                title: b.title || 'Beat Instrumental',
                audioUrl: b.audioUrl,
                coverUrl: b.coverUrl || '/assets/dark_brick_graffiti.jpg',
                isRadioInterstitial: false,
                fullName: b.fullName || b.title,
                bpm: b.bpm
              }));
            }
          }
        ...
        console.warn("[TURNTABLE] /api/beats-local endpoint unavailable:", e);
      }

      // B. Fetch Firestore tracks marked as beats
      try {
        const qFallback = query(collection(db, 'tracks'), limit(50));
        const snapFallback = await getDocs(qFallback);
        const dbTracks = snapFallback.docs
          .map(d => ({ id: d.id, ...d.data() } as any))
          .filter(t => t.isRadioInterstitial !== true && (
            t.isBeat === true || 
            t.genre?.toLowerCase() === 'beat' || 
            t.genre?.toLowerCase() === 'instrumental' || 
            t.title?.toLowerCase().includes('beat') ||
            t.title?.toLowerCase().includes('instrumental')
          ));

        const seenUrls = new Set(combinedBeats.map(t => t.audioUrl.toLowerCase()));
        dbTracks.forEach(dbt => {
          if (dbt.audioUrl && !seenUrls.has(dbt.audioUrl.toLowerCase())) {
            combinedBeats.push({
              id: dbt.id,
              artistId: dbt.artistId || 'artist',
              artistName: dbt.artistName || 'RAPLIFE ARTIST',
              title: dbt.title || 'Track Instrumental',
              audioUrl: dbt.audioUrl,
              coverUrl: dbt.coverUrl || '/assets/dark_brick_graffiti.jpg',
              isRadioInterstitial: false,
              fullName: dbt.title || dbt.id,
              bpm: dbt.bpm
            });
          }
        });
      } catch (innerErr) {
        console.warn("[TURNTABLE] Firestore beats query fallback error:", innerErr);
      }

      // C. Apply custom beats order if saved
      ...
      if (beatOrder.length > 0) {
        ...
        beatOrder.forEach((identifier, index) => {
          orderMap.set(identifier, index);
        });

        combinedBeats.sort((a, b) => {
          const idA = (a as any).fullName || a.id;
          const idB = (b as any).fullName || b.id;
      ...
      setTracks(combinedBeats);
      
      if (combinedBeats.length > 0) {
        setActiveBeat((prev) => {
          if (!prev) return combinedBeats[0];
          const exists = combinedBeats.find(b => b.id === prev.id || b.audioUrl === prev.audioUrl);
          return exists || combinedBeats[0];
        });
      }
    } catch (err) {
        ...
        <div className="mt-10 border-t border-neutral-800 pt-8">
          <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-6">
            ...
            <button
              ...
              className="flex items-center gap-1.5 px-3 py-1.5 bg-neutral-800 hover:bg-neutral-700 text-gray-300 hover:text-white rounded-lg font-mono text-[10px] font-bold uppercase transition-all self-start sm:self-auto cursor-pointer"
              title="Volver a escanear carpeta assets y base de datos"
            >
              <RotateCcw size={12} className={loadingTracks ? 'animate-spin' : ''} />
              <span>{loadingTracks ? 'CARGANDO...' : 'REFRESCAR BEATS'}</span>
            </button>
          </div>
