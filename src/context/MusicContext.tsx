...
export const MusicProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  const play = (track: Track) => {
    ...
    const targetAudibleVolume = (volume !== undefined && volume >= 0.15) 
      ? volume 
      : (previousVolumeRef.current >= 0.2 ? previousVolumeRef.current : 1.0);
    audio.muted = false;
    audio.volume = targetAudibleVolume;
    setVolumeState(targetAudibleVolume);
    setIsMuted(false);
    try { localStorage.setItem('raplife_radio_muted', 'false'); } catch (_) {}
    try { localStorage.setItem('raplife_radio_volume', targetAudibleVolume.toString()); } catch (_) {}
